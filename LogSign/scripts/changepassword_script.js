...
function validateReEnterPassword() {
    ...
    var newpass = document.getElementById("enter_password").value;
    var reset_password = document.getElementById("reenter_password").value
...
function validateLogin() {
    ...
    if (!validateNewPassword() || !validateReEnterPassword()) {
        ...
        var password = document.getElementById("reenter_password").value;

        $.ajax({
            type: "POST", //type of method
            url: "update-pass.php", //your page
            data: { password: password }, // passing the values
            success: function(res) {

                if (res == 200) {
                    //dapat to hindi ganito eh pero no choice pansamantala muna
                    alert("Success! You will now redirect to login page");
                    window.location.href = "log_sign.html";
                } else {
                    console.log(res);
                    document.getElementById("reenter_password-error").innerHTML = "Something Goes Wrong!";
                }
